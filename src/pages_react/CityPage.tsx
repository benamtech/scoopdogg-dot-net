...
function FAQItem({ q, a }: { q: string; a: string }) {
  // <details> instead of useState: the answer is in the HTML whether or not it is open,
  // and it works with JavaScript off. `{open && <p>…}` kept FAQ answers out of the
  // document entirely, which on a site whose FAQs are half its long-tail content is
  // throwing away the reason the page ranks.
  return (
    <details className="group border-b border-sage-light last:border-0">
      <summary className="w-full flex items-center justify-between py-4 text-left font-medium text-dark hover:text-forest transition-colors cursor-pointer list-none">
        {q}
        <ChevronDown
          size={18}
          className="flex-shrink-0 ml-4 transition-transform text-dark/40 group-open:rotate-180 group-open:text-forest"
        />
      </summary>
...
export default function CityPage({ city: citySlug }: { city?: string } = {}) {
        ...
        <div className="max-w-site mx-auto px-4 sm:px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <Link
              ...
              className="inline-flex items-center justify-center gap-2 bg-amber hover:bg-amber-hover text-dark font-semibold px-7 py-3.5 rounded-full transition-all hover:scale-[1.02] hover:shadow-lg text-base"
            >
        ...
        <div className="max-w-site mx-auto px-4 sm:px-6">
          ...
          <div className="bg-forest rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
              <Link
                to={`/book?city=${encodeURIComponent(city.name)}`}
                className="inline-flex items-center justify-center gap-2 bg-amber hover:bg-amber-hover text-dark font-semibold py-3 px-7 rounded-full transition-all hover:scale-[1.02] hover:shadow-md whitespace-nowrap"
              >
                Book in 60 Seconds →
              </Link>
              <a
        ...
        <div className="max-w-site mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-4 -left-4 bg-amber text-dark rounded-2xl p-4 shadow-lg">
