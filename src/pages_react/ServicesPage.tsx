...
const MASCOT_MAP: Record<string, { src: string; alt: string }> = {
  'weekly-yard-maintenance': { src: '/img/transp-dog-character-mowing-grass.png', alt: 'Dog character mowing grass' },
  'kitty-litter-exchange': { src: '/img/transp-cat-character-cleaning-litter-box.png', alt: 'Cat character cleaning litter box' },
  'cat-tree-cleaning': { src: '/img/transp-cat-character-cleaning-cat-tree.png', alt: 'Cat character cleaning cat tree' },
  'pressure-washing': { src: '/img/transp-dog-character-pressure-washing-driveway.png', alt: 'Dog character pressure washing driveway' },
  'kitty-litter-robot-cleaning': { src: '/img/5878.jpg', alt: 'Litter-Robot automatic litter box' },
};

/**
 * Whole sentences up to a target length, so a card never ends mid-word and never ends
 * after three words either. A clamped paragraph cut "haul it…" and "persistent odo…";
 * one sentence alone left "Our most popular service." sitting on an empty card.
 */
function cardSummary(text: string, target = 150) {
  const sentences = text.match(/[^.!?]+[.!?]+(?=\s|$)/g) ?? [text];
  let out = '';
  for (const sentence of sentences) {
    if (out && (out + sentence).trim().length > target) break;
    out = (out + sentence).trimStart();
    ...
    <div className="bg-cream">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    {mascot && (
                      <img
                        ...
                        loading="lazy"
                        decoding="async"
                        className="hidden md:block ml-auto h-14 w-14 object-contain flex-shrink-0 pointer-events-none select-none"
                      />
                    )}
                  </div>
                  <p className="text-dark/60 text-sm leading-relaxed mb-5 flex-1">
                    {cardSummary(service.intro)}
                  </p>
      ...
      <section className="py-16 bg-forest">
        <div className="max-w-site mx-auto px-4 sm:px-6 text-center">
          ...
          <Link
            to="/book"
            className="inline-flex items-center gap-2 bg-amber hover:bg-amber-hover text-dark font-semibold px-8 py-4 rounded-full transition-all hover:scale-[1.02] hover:shadow-lg"
          >
